...
<div class="panel panel-primary">
	<div class="panel-heading" style="background: #15A638;">
		<h3 class="panel-title" align="center">路人影视系统安装</h3>
	</div>
...
if(defined("SAE_ACCESSKEY") || $_GET['jump']==1){
	//if(defined("SAE_ACCESSKEY"))include_once '../includes/sae.php';	else 
	...
	if(!$dbconfig['user']||!$dbconfig['pwd']||!$dbconfig['dbname']) {
		...
		if(!$con=DB::connect($dbconfig['host'],$dbconfig['user'],$dbconfig['pwd'],$dbconfig['dbname'],$dbconfig['port'])){
			...
			if(DB::query("select * from tyys_user where 1")==FALSE)
				echo '<p align="right"><a class="btn btn-primary btn-block" href="?do=4">创建数据表>></a></p>';
			else
				echo '<div class="list-group-item list-group-item-info">系统检测到你已安装过爱微宝影视系统</div>
...
?>";
		if(!$con=DB::connect($db_host,$db_user,$db_pwd,$db_name,$db_port)){
			...
			if(DB::query("select * from tyys_user where 1")==FALSE)
				...
				echo '<div class="list-group-item list-group-item-info">系统检测到你已安装过路人影视系统</div>
				<div class="list-group-item">
					<a href="?do=6" class="btn btn-block btn-info">跳过安装</a>
				</div>
				<div class="list-group-item">
...
<?php
	...
	clearpack();
	echo '<div class="alert alert-info"><font color="green">安装完成！用户管理账号和密码是:tyyule/123456</font><br/><br/><a href="../">>>网站首页</a>｜<a href="../user/">>>用户管理</a> <hr/>更多设置选项请登录后台管理进行修改。<br/><br/><font color="#FF0033">如果你的空间不支持本地文件读写，请自行在install/ 目录建立 install.lock 文件！</font></div>';
?>
	</div>
</div>
