...
<?php
    ...
    banner();
if($type=="va"){
    $zongyi= getSubstr($info, '<div id="js-year-all" class="js-month-tab">', '<div class="juji-page">');  //综艺。。
    ...
    preg_match_all('#<a data-daochu="to=(.*?)" class="(.*?)" href="(.*?)">(.*?)</a>#', $dianyingth,$movie); //站点排序：
    $mrsp=$movie[3][0];
...
<?php if(empty($mrsp)){ ?>
    <div class="am-alert am-alert-warning" data-am-alert>
		...
		Sorry! 	<?php if($title!="404"){?>  该片暂无资源. 你可以看看<a href="/zy/<?=$title;?>.html" target="_blank">[资源搜索]</a>里有没有<?php }else{?> 无效的URL，请不要胡乱输入哦！<?php }?> 
    </div>

<?php }else{ ?>
    <div class="am-panel-bd player-box">
        <iframe id="ty-vip-player" src="" width="100%" height="100%" allowfullscreen="true" allowtransparency="true"></iframe>
    </div>
...
<div class="am-panel am-panel-default">
    ...
    $moviehref=$movie0;
     echo '
     <div alt="TY影视" class="am-btn-group play-source-group">
...
function tyjiexi(checked) {
        ...
        layer.msg( checked?"已启用 点击下面与芒果相关的视频吧！(目前只支持芒果视频的解析哦！)":"已关闭" );
}
...
function refreshVideo() {
	if(store.get('tyjx')==true && videoInfo.url.indexOf("www.mgtv.com/b/")!=-1){
		$("#ty-vip-player").attr("src", 'http://jx.tyjun.cn?url=' + videoInfo.url);
	}else{
