...
<?php 
$dslb=getSubstr($info,'<div class="p-mod dianshi">', '</ul>');

$tvname='#<span class=\'s1\'>(.*?)</span>#';
$tvping='#<span class=\'s2\'>(.*?)</span>#';
$tvlist="#<a href='(.*?)'  class='js-link'>#";
...
preg_match_all('#<a title="(.*?)" href="(.*?)" class="name">(.*?)</a>#', $dylb,$vtop); 
preg_match_all('#<span class="vv">(.*?)</span>#', $dylb,$vbfl); 
     foreach ($vtop[3] as $i => $dyname)
       {   $k=$i+1;
...
preg_match_all('#<a title="(.*?)" href="(.*?)" class="name">(.*?)</a>#', $dslb,$tvtop); 
preg_match_all('#<span class="vv">(.*?)</span>#', $dslb,$tvbfl); 
     foreach ($tvtop[3] as $i => $dsname)
       {   $k=$i+1;
